...
const select_container = document.querySelectorAll("#contain_hover");
...
if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
  body_theme.classList.toggle("body_theme");
  change_button_color = "rgb(85, 85, 85)"
}

function theme_mode(){
  if(Theme.value == 1){
    body_theme.classList.remove("body_theme");
    change_button_color = "#e6e6e6";    
  }
  else{
    body_theme.classList.add("body_theme");
    change_button_color = "rgb(85, 85, 85)"
  }
}
...
select_container.forEach(function(select_container) {
    ...
    select_container.addEventListener("mouseenter", function() {
        select_container.children[1].style.backgroundColor = change_button_color;
    });
